...
private:
	...
	double calcNewWeight(double , NeuralNet, double, double, double);
...
inline std::vector<Neuron> Training::teachNeuronsOfLayer(size_t numberOfInputNeurons, size_t line, NeuralNet &n, double netValue)
{
	...
	for (size_t j = 0; j < numberOfInputNeurons; j++) {
		...
		inputWeightsInNew.push_back(this->calcNewWeight(inputWeightOld, n, this->getError(), n.getTrainSet()[line][j], netValue));
...
inline double Training::calcNewWeight(double inputWeightOld, NeuralNet n, double error, double trainSample, double netValue)
{
